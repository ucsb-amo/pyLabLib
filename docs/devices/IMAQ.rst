...
- Some frame grabbers can only operate in 32bit mode, and could raise ``IMG_ERR_32_BIT_MEMORY_LIMITATION`` error on a 64bit OS. In this case, one needs to enable legacy 32bit memory allocation mode by adding ``cam._buffer_mgr.legacy_32bit_mode=True`` right after the camera object creation.